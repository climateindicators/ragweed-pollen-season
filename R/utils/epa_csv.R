...
# indicator-agnostic and is meant to be reused verbatim for the other
# indicators:
#
#   line 1  figure title
...
#   line 8+ data
#
# Two traps this file exists to absorb:
#
# 1. The files are windows-1252, not UTF-8. On Windows, readLines(encoding=)
#    only *tags* a string, it does not convert it, so the conversion has to be
...
#'
#' @return list(title, source, data_source, web_update, units)
read_epa_preamble <- function(path) {
  raw <- readLines(path, n = 5L, warn = FALSE)
  txt <- iconv(raw, from = "windows-1252", to = "UTF-8")
  if (anyNA(txt)) {
    stop("Preamble of ", basename(path), " is not valid windows-1252.", call. = FALSE)
  }
  ...
  list(
    title       = txt[1],
    source      = drop_label(txt[2], "Source"),
    data_source = drop_label(txt[3], "Data sources?"),
    web_update  = drop_label(txt[4], "Web update"),
...
#' means instead of readr guessing.
read_epa_csv <- function(path) {
  readr::read_csv(
    path,
    skip        = EPA_CSV_SKIP,
...
#' EPA's own published Figure 2 plots these as zero, which reads as "no deaths".
#'
